...
class QLearning():
	...
	def get_best_action(self, state):
		...
		for (s, a), value in self.q_dict.items():
			if s == state and value > max_value:
				max_value = value
				best_action = a
	...
	def get_value_from_q_dict(self, state, action):
		q_value = self.q_dict.get((state, action))
		if q_value != None:
			return q_value
		else:
		...
		for episode in range(num_episodes):
			...
			for step in range(max_steps_per_episode):
				...
				if expl_threshold > exploration_rate:
					...
					action, max_value = self.get_best_action(state)
				...
				new_action, max_value = self.get_best_action(new_state)
				self.q_dict[(state, action)] = (1 - alpha) * self.get_value_from_q_dict(state, action) + alpha * (reward + discount_rate * max_value)
